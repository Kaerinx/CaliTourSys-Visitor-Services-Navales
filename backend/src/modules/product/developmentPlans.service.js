...
function normalizeDevelopmentPlanInput(input = {}) {
  return {
    ...
    planTitle: String(input.planTitle || input.title || '').trim(),
    objectives: String(input.objectives || '').trim(),
    targetMarket: String(input.targetMarket || '').trim(),
    improvementNeeds: String(input.improvementNeeds || 'Not specified').trim(),
    ...
    timelineStartTime: String(input.timelineStartTime || '').trim(),
    timelineEndTime: String(input.timelineEndTime || '').trim(),
