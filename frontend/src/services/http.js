...
function getVisitorToken() {
  ...
  return (
    window.localStorage.getItem('calitoursys_tourist_access_token') ||
    window.localStorage.getItem('calitoursys_token') ||
...
async function apiRequest(
  ...
  if (!response.ok || payload?.success === false) {
    const error = new ApiError(payload?.error?.message || payload?.message || 'The tourism API request failed.', {
      status: response.status,
  ...
  if (payload?.success !== true || !Object.prototype.hasOwnProperty.call(payload, 'data')) {
    if (path.startsWith('/visitor')) return payload
...
export async function request(path, options = {}) {
  ...
  const legacyToken = getVisitorToken()
  try {
