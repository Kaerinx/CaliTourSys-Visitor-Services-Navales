...
const { relations, relationError } = useCmsRelations({
  ...
  businesses: () => cmsContentApi.getBusinesses({ limit: 100, status: 'active', sort: 'name' }),
  accreditedEstablishments: () => cmsContentApi.getProductAccreditedEstablishments({ limit: 100 }),
})
...
const accreditedEstablishmentOptions = computed(() =>
  (relations.value.accreditedEstablishments || []).map((establishment) => ({
    ...establishment,
    label: establishment.recordNumber
      ? `${establishment.businessName} - ${establishment.recordNumber}`
      : `${establishment.businessName} - Accredited`,
  })),
)
...
const productImageUrl = (item) => item.primaryImage?.url || item.imageUrl || ''
...
async function submitRecord(payload) {
  ...
  formError.value = ''
  try {
    const body = productPayload(payload)
    if (selected.value?.id) await cmsContentApi.updateProduct(selected.value.id, body)
    else await cmsContentApi.createProduct(body)
...
<template>
  <section class="cms-content-page" aria-labelledby="cms-products-title">
    ...
    <CmsDataTable :columns="columns" :items="items" :loading="isLoading" :error="error" empty-title="No products found" empty-text="Create a product draft or adjust your filters." @retry="load">
      <template #rows="{ items: rows }"><tr v-for="item in rows" :key="item.id"><td><span class="cms-product-cell"><span class="cms-product-thumb" aria-hidden="true"><img v-if="productImageUrl(item)" :src="productImageUrl(item)" :alt="item.name" /><span v-else>No image</span></span><span class="cms-table-title"><strong>{{ item.name }}</strong><span>{{ item.slug }}</span></span></span></td><td>{{ item.business?.name || 'No business' }}</td><td>{{ item.category?.name || 'No category' }}</td><td>{{ money(item) }}</td><td><CmsStatusBadge :status="item.status" /></td><td><span class="cms-table-actions"><button v-if="can('products.update')" type="button" @click="openEdit(item)">Edit</button><button v-if="can('products.publish') && item.status !== 'published'" type="button" @click="askAction('publish', item)">Publish</button><button v-if="can('products.archive') && item.status !== 'archived'" class="is-danger" type="button" @click="askAction('archive', item)">Archive</button></span></td></tr></template>
      <template #cards="{ items: cards }"><article v-for="item in cards" :key="item.id" class="cms-mobile-card"><span class="cms-product-cell"><span class="cms-product-thumb" aria-hidden="true"><img v-if="productImageUrl(item)" :src="productImageUrl(item)" :alt="item.name" /><span v-else>No image</span></span><span class="cms-table-title"><strong>{{ item.name }}</strong><span>{{ item.slug }}</span></span></span><div class="cms-mobile-meta"><CmsStatusBadge :status="item.status" /><span>{{ money(item) }}</span></div><span>{{ item.business?.name || 'No business' }} / {{ item.category?.name || 'No category' }}</span><div class="cms-mobile-card__actions cms-table-actions"><button v-if="can('products.update')" type="button" @click="openEdit(item)">Edit</button><button v-if="can('products.publish') && item.status !== 'published'" type="button" @click="askAction('publish', item)">Publish</button><button v-if="can('products.archive') && item.status !== 'archived'" class="is-danger" type="button" @click="askAction('archive', item)">Archive</button></div></article></template>
    </CmsDataTable>
    <CmsPagination :pagination="pagination" @page-change="setPage" />
    <CmsProductForm :open="formOpen" :value="selected" :categories="relations.categories || []" :accredited-establishments="accreditedEstablishmentOptions" :busy="isSaving" :server-error="formError" @close="formOpen = false" @submit="submitRecord" />
