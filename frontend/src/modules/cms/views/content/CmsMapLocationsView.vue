...
const { relations, relationError } = useCmsRelations({
  ...
  experienceOptions: () => cmsContentApi.getMapLocationExperienceOptions(),
})
...
async function openEdit(item) {
  ...
  formError.value = ''
  try {
    const { data } = await cmsContentApi.getMapLocationById(item.id)
    selected.value = data
    formOpen.value = true
  } catch (err) {
...
async function submitRecord({ location, experience }) {
  isSaving.value = true
  formError.value = ''
  const wasEditing = Boolean(selected.value?.id)
  try {
    let record
    if (wasEditing) {
      const response = await cmsContentApi.updateMapLocation(selected.value.id, location)
      record = response.data
    } else {
      const response = await cmsContentApi.createMapLocation(location)
      record = response.data
      selected.value = record
    }
    if (experience) await cmsContentApi.updateMapLocationExperience(record.id, experience)
    formOpen.value = false
    notice.value = wasEditing ? 'Map location and detail content updated.' : 'Map location and detail content created.'
    await refreshViews()
  } catch (err) {
    formError.value = friendlyContentError(err)
    if (!wasEditing && selected.value?.id) {
      formError.value = `The map pin was created, but its detail content could not be saved. Review the fields and save again. ${formError.value}`
    }
  } finally {
...
<template>
  <section class="cms-content-page" aria-labelledby="cms-map-title">
    ...
    <template v-if="activeView === 'table'">
      <CmsDataTable :columns="columns" :items="items" :loading="isLoading || isOpening" :error="error" empty-title="No map locations found" empty-text="Create a map location or adjust your filters." @retry="load">
    ...
    <CmsMapLocationForm :open="formOpen" :value="selected" :initial-coordinates="initialCoordinates" :destinations="relations.destinations || []" :businesses="relations.businesses || []" :events="relations.events || []" :activities="relations.experienceOptions?.activities || []" :packages="relations.experienceOptions?.packages || []" :media-assets="relations.experienceOptions?.mediaAssets || []" :busy="isSaving" :server-error="formError" @close="formOpen = false" @submit="submitRecord" />
