...
async function getLegacyProductApi(path) {
  ...
  if (!response.ok) {
    const error = new Error(
      data?.message || data?.error?.message || "Product API request failed",
    );
...
export function getEmergencyFacilities() {
  return http.get(publicPath("/map/emergency-facilities"));
}

export function getMapLocationDetails(id, options = {}) {
  return http.get(
    publicPath(`/map/locations/${encodeURIComponent(id)}/details`),
    undefined,
    options,
  );
}
